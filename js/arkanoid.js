...
start.classList.add("btn", "btn-lg", "btn-success");
    ...
    for (let c = 0; c < columnCount; c++) {
        for (let r = 0; r < rowCount; r++) {
            if (bricks[c][r].status === 1) {
                ...
                bricks[c][r].y = brickY;
                ctx.beginPath();
                ctx.rect(brickX, brickY, brickWidth, brickHeight, 30);
                ctx.fillStyle = 'blue';
                ctx.fill();
                ctx.closePath();
            }
...
start.addEventListener("click", start_stop_game, false);
